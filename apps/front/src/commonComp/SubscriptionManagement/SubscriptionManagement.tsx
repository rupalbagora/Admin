...
const SubscriptionManagement = () => {
	...
	const { plans, loading, error, currentPlan } = useAppSelector(
	...
	const handleSubmit = async () => {
		try {
			if (isEditMode) {
				await dispatch(
					updateSubscriptionPlan({ id: formData._id, data: formData })
				).unwrap();
				setSnackbar({
					open: true,
	...
	const handleSetCurrent = (plan: SubscriptionPlan) => {
		...
		setSnackbar({
			open: true,
			message: `Current plan set to ${plan.name}`,
			severity: "info",
		});
	};
								...
								<TableRow key={plan._id}>
									...
									<TableCell>
										...
										<Button
											...
											color="primary"
											variant={
												currentPlan?._id === plan._id ? "contained" : "outlined"
											}
