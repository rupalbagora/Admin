...
const ManageProductPackages: React.FC = () => {
  ...
  const columns = [
    ...
    { title: "Usage", dataIndex: "usage", key: "usage" },
    {
      ...
      dataIndex: "image",
      key: "image",
      render: (image: string) => image ? <img src={image} alt="Package" className="w-16 h-16 object-cover" /> : "No image",
    },
    {
      title: "Actions",
      key: "actions",
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<EditOutlined />} onClick={() => { setEditingPackage(record); setIsModalOpen(true); }}>Edit</Button>
  ...
  return (
    <div className="p-6">
      <Card
        ...
        extra={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditingPackage(null); setIsModalOpen(true); }} className="bg-gray-700">
            Add Product Package
          </Button>
        }
      >
        <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <Search placeholder="Search product packages..." value={searchText} onChange={(e) => setSearchText(e.target.value)} allowClear size="large" className="w-full md:w-1/2 bg-[#2523232c]" />
        </div>
      ...
      >
        <ProductPackageForm
          ...
          onSuccess={() => { setIsModalOpen(false); dispatch(fetchProductPackages()); }}
        />
      </Modal>
    </div>
  );
